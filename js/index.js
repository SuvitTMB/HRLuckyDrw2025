...
function OpenRandom() {
  ...
  .get().then((snapshot)=> {
  snapshot.forEach(doc=> {
    //CheckOpen = doc.data().RandomStatus;
    if(doc.data().RandomStatus==0) {
      location.href = "ttb.html";
    } else if(doc.data().RandomStatus==1) {
      sessionStorage.setItem("OpenOSRandom2", doc.data().RandomStatus);
      CheckData();
    } else if(doc.data().RandomStatus==2) {
      location.href = "thank.html";
    }
    //alert(doc.data().RandomStatus);
    }); 
  });
}
...
function CheckProfile() {
  ...
  NewEmpNumber = "ttb"+txtEmpNumber.slice(3, 13);
  //alert(NewEmpNumber);
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      if(doc.data().LineID=="") {
        ...
        str += '<div style="font-size:13px; margin-top:20px;"><b><u>คุณสมบัติผู้เข้าร่วมโครงการ</u></b><br><ul><li>พนักงานสัญญาจ้างภายนอก ที่มีอายุงานครบ 1 ปีต่อเนื่องขึ้นไป ณ วันที่ 31 ธันวาคม 2567  (เข้างานก่อนหรือในวันที่ 1 มกราคม 2567)</li><li>มีสถานะเป็น พนักงานสัญญาจ้างภายนอก ของทีทีบี ณ วันที่มอบรางวัล</li></ul></div>';
        ...
        str += '<div style="font-size:13px;"><ul><li><u>โชคชั้นที่ 1</u> ลุ้นรับเงินสดมูลค่า 1,000 - 4,000 บาท</li><li><u>โชคชั้นที่ 2</u> ลุ้นรับของรางวัลใหญ่ 3 รางวัล<br>+ รางวัลที่ 1 ทองคำหนัก 1 บาท จำนวน 1 รางวัล<br>+ รางวัลที่ 2 ทองคำหนัก 0.50 บาท จำนวน 1 รางวัล<br>+ รางวัลที่ 3 ทองคำหนัก 0.25 บาท จำนวน 1 รางวัล</li></ul></div>';
        ...
        str += '<br><br><b><u>ระยะเวลาการร่วมกิจกรรม</u></b><br>ระหว่างวันที่ 20 ม.ค. 68 เวลา 09:00 น. - 21 ม.ค. 2568 เวลา 17.00 น. เท่านั้น';
        str += '<br><br><b><u>การมอบรางวัล</u></b><ul style="margin-left:-20px;"><li>สำหรับโชคชั้นที่ 1  ท่านจะได้รับ<u>รางวัลเงินสด</u> เข้าบัญชีที่ท่านรับเงินเดือน ในวันที่ 29 ม.ค. 2568 (จ่าย 1 รอบ)</li><li>สำหรับโชคชั้นที่ 2 ทรัพยากรบุคคลจะติดต่อผู้โชคดีที่ได้<u>รับรางวัลใหญ่ 3 รางวัล</u> ภายใน 29 ม.ค. 2568</li></ul>';
...
function ConfirmRegister() {
  ...
  if(xtxtEmpAccept == true) { 
    ...
    str += '<div style="margin-top:20px;text-align:left;"><font color="#ffff00"><b>คุณ'+ sessionStorage.getItem("EmpName_HR")+'</b> ได้รับสิทธิจับฉลากอั่งเปาโชค 2 ชั้น อย่างละ 1 สิทธิ ระหว่างวันที่ 20 ม.ค. 68 เวลา 09:00 น. - 21 ม.ค. 2568 เวลา 17.00 น. เท่านั้น</font><br><br>ธนาคารสงวนสิทธิ์ในการกำหนด เปลี่ยนแปลง หรือยกเลิกหลักเกณฑ์ใด ๆ ตามความเหมาะสม และการตัดสินใจของธนาคารถือเป็นที่สุด</div>';
