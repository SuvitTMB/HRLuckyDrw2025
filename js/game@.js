...
$(document).ready(function () {
  ...
  Connect_DB();
  //dbReward = firebase.firestore().collection("RSOCtownhall");
...
function CheckData() {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      gcheck = 1;
      Eid = doc.id;
      ShowGift = doc.data().TypeRandom;
      ShowPrize = doc.data().TypeName;
      ...
      //console.log("Found : "+gcheck+" --- "+ doc.data().TypeRandom+" --- "+ doc.data().TypeName );
      document.getElementById('loading').style.display='none';
      document.getElementById('ShowWheel1').style.display='block';
      str += '<center>';
      if(doc.data().TypeRandom=="gift-99") {
        str += '<div style="margin:50px auto 10px auto;"><img src="./img/'+ doc.data().TypeRandom +'.png" style="width:220px;"/></div>';
        ...
        if(doc.data().StatusSend==1) {
          str += '<div class="boxtext" style="margin-top:20px; color:#000;">ยินดีด้วยคุณได้รับรางวัล<br><b>'+ doc.data().TypeName +'</b></div>';
          ...
          str += '<div class="boxtext" style="margin-top:20px; color:#000;">ยินดีด้วยคุณได้รับรางวัล<br><b>'+ doc.data().TypeName +'</b><br><br>คุณได้รับรางวัลเรียบร้อยแล้ว<br>'+ doc.data().SendGift +'</div>';
...
function RandomRewards() { 
  var i = 0;
  Eidewards = "";
  dbReward.where('LineID','==','')
  .get().then((snapshot)=> {
    snapshot.forEach(doc=>{
      ArrRewards.push([doc.id, doc.data().TypeName, doc.data().TypeRandom ]);
    });
